...
- **Reuse-first — always check existing tags before suggesting.** Run `agentteams document tags` to see the project's existing tags (with usage counts) and prefer reusing them. Inventing near-duplicates (`기획` vs `기획안`) fragments the tree, so match an existing tag whenever one fits.
- **Use `/` for hierarchy.** A tag like `업무/기획/AI` renders as a nested folder (`업무 > 기획 > AI`) in the document tree. Keep paths shallow and consistent.
