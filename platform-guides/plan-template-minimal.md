# Plan Template (MINIMAL Tier)

> ⚠️ This file is automatically deployed from the server. Do not edit it directly.

> This template is for **MINIMAL complexity** plans (1 task · 1–2 files · single domain · no risk signals).
> Set `--complexity MINIMAL` at creation. V2 plans render this structured content directly in the web UI.
> Larger scope? Use `plan-template-standard.md` or `plan-template-full.md` — the tier rule is in `plan-authoring-guide.md` (**Plan Complexity**). Do not add FULL-tier sections here; if the plan needs them, it is not a MINIMAL plan.

## TL;DR

> **In Plain Terms**: <!-- 1-2 sentences a non-expert (the requester/stakeholder) can read to confirm the plan matches their intent. State the problem and what visibly changes for the user. Write in the plan's language. NO file paths, code identifiers, or internal jargon (e.g. SSOT, projectId). See plan-authoring-guide.md → TL;DR Audience. -->
>
> **Quick Summary**: <!-- 1-2 sentence technical summary of what this plan achieves -->
>
> **Deliverables**:
>
> - <!-- file/module — what changes -->

---

## Conventions Referenced

<!--
Required at every tier. List the project convention files and skill packages you
actually consulted while drafting this plan — do not guess. Same format as completion
reports so the platform can auto-link plan-stage usage to execution-stage usage.

Example:
- .agentteams/rules/<convention-name>.md
- .agentteams/rules/<convention-name>.md - <why it was relevant>
- .agentteams/skills/<slug>/SKILL.md - <why the package was needed>
-->

- <!-- convention file — why it was relevant -->

---

## TODOs

> **Structured task contract**: Keep the exact `## TODOs` heading and numbered `### N. Task title` headings. The server parses dependencies from `Blocked By: Task N` or `Depends On: Task N`, and waves from `Parallel Group: Wave N`. Reference only task numbers that exist in this plan.

---

### 1. Task title

**What to do**:

  <!-- Detailed steps. If a specific you could not verify against this repo matters here, say so inline rather than stating it as fact. -->

**Acceptance Criteria**:

- <!-- Verifiable criterion — the SSOT for this task being complete: the evidence required before marking the task `DONE`; prefer a test assertion -->
